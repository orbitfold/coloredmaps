...
@click.option('-n', '--neighbors', help='How many neighbors')
def main(input_dir, dataset, neighbors):
  dataset = pd.read_csv(dataset)
  neigh = KNeighborsClassifier(n_neighbors=3)
  neigh.fit(np.array([dataset.r, dataset.g, dataset.b]).transpose(), dataset['class'])
  confusion = {
  ...
  for c_map in COLORED:
    ...
    with open(os.path.join(input_dir, f"{stem}", f"{stem}.json"), 'r') as fd:
      ...
      colored = 0
      for key in data:
        prediction = neigh.predict([data[key]['center']])
        if prediction[0] == 2:
          colored += 1
      if colored > 0:
